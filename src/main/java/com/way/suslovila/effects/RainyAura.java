...
public class RainyAura extends MobEffect {
    ...
    public void applyEffectTick(LivingEntity pLivingEntity, int pAmplifier) {
        ...
        if (pLivingEntity.hasEffect(ModEffects.HELLISH_FLAMES.get())) pLivingEntity.removeEffect(ModEffects.HELLISH_FLAMES.get());
...
//            }
            ...
            if (ClientRainyAuraData.getBlocks() != null) {
                ...
                for (int g = 0; g < 2; g++) {
                    for (int i = 0; i < blocksFromCap.size(); i++) {
                        ...
                        pLivingEntity.level.playSound(null, pos.getX(), pos.getY(), pos.getZ(), SoundEvents.WEATHER_RAIN, SoundSource.AMBIENT, 1.0F, 1.0F);
        ...
        if (!pLivingEntity.level.isClientSide()) {
            if (pLivingEntity.getCapability(RainyAuraCapProvider.BLOCKS).isPresent()) {
                ...
                HashMap<UUID, Integer> entitiesFromCap = pLivingEntity.getCapability(RainyAuraCapProvider.BLOCKS).map(RainyAuraStorage::getEntities).get();
...
//            }
            ...
            if (pLivingEntity.getCapability(RainyAuraCapProvider.BLOCKS).isPresent()) {
                ...
                Iterator<BlockPos> iteratorBlock = blocksFromCap.keySet().iterator();
                    ...
                    if (pos != null) {
                        ...
                        if (!(block == Blocks.FIRE || block == Blocks.SOUL_FIRE || (block == Blocks.LAVA && pAmplifier >= 2))) {
                            ...
                            pLivingEntity.getCapability(RainyAuraCapProvider.BLOCKS).ifPresent(blocks -> {
                                ...
                                if (Math.sqrt(pLivingEntity.blockPosition().distToCenterSqr(pos.getX(), pos.getY(), pos.getZ())) > 7) {
                                    blocks.removeBlock(pos);
